...
for i = 1:4 
    ...
    transmit_data = conv(data_upsampled, filter,'same');
    receive_data = conv(transmit_data, filter,'same');  
    ...
    eye_diagram_I = eyediagram(eye_diagram_data(1,:), n_samples * 2); % In-phase
    title("Eye Diagram for In-Phase Signal when R="+R_cases(i)+" & delay="+delay_cases(i)+"");
 
    eye_diagram_Q = eyediagram(eye_diagram_data(2,:), n_samples * 2); % Quadrature
    title("Eye Diagram for Quadrature Signal when R="+R_cases(i)+" & delay="+delay_cases(i)+"");
end
